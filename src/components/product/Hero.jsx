...
import {
  Controller, EffectFade, EffectCube, Autoplay,
} from "swiper";
import "swiper/css";
          ...
          >
            {productPoints.map((patner, i) => (
              <SwiperSlide key={i} className="flex justify-center items-center relative">
                <CustomIMG src={patner.image} alt="mask" className="lg:w-max sm:w-96 w-72" />
              </SwiperSlide>
            ))}

          </Swiper>
          <h2 className="absolute lg:-bottom-48 sm:-bottom-0 -bottom-20 lg:text-3xl font-semibold left-1/2 max-md:-translate-x-1/2 lg:-translate-x-12">Skyted Mask</h2>
        </div>
          ...
          <ul className="space-y-10 lg:mt-28">
            {
              productPoints.map((point) => (
                ...
                >
                  <span className={`" ${point.id == activeSlide ? "p-0" : "p-1"} transition-all  bg-gray-300 flex justify-center items-center w-5 h-5 rounded-full m-0 "`}>
                    <span className="w-full h-full block rounded-full group-hover:w-full group-hover:h-full transition-all" style={{ backgroundColor: point.color }} />
                  </span>
                  <p className="max-lg:text-sm">{point.title}</p>
                </li>
              ))
            }
          </ul>
