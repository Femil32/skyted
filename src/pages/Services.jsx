		...
		<>
			<main className="xl:py-16 pt-12 pb-0 bg-white">
				{serviceData?.map((service, i) => {
					const { ServiceTitle, ServiceSubtitle, ServiceDescription } = service;
					const { url } = service?.ServiceImage?.data?.attributes;
					return (
						<section
							className={`${i % 2 === 0 ? "bg-white" : "bg-[#F7FAFB]"} text-black section-container`}
							key={i}
						>
							<div>
								...
								<ImgData
									...
									description={
										<>
											<div className="custom-ul">{parseHtml(service?.ServiceDescription)}</div>
